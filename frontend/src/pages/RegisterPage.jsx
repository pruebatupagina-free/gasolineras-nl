...
export default function RegisterPage() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      toast.success('¡Cuenta creada! Bienvenido a GasoNL')
      navigate('/app')
    } catch (err) {
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'var(--color-bg)', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '20%', left: '50%', transform: 'translate(-50%,-50%)', width: 500, height: 500, background: 'radial-gradient(circle, rgba(34,197,94,0.08) 0%, transparent 70%)', pointerEvents: 'none' }} />
      ...
      <div className="glass-card animate-slide-up" style={{ width: '100%', maxWidth: 440, padding: 40 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 24, fontWeight: 700, marginTop: 24, marginBottom: 8 }}>Crea tu cuenta gratis</h1>
          <p style={{ color: 'var(--color-muted)', fontSize: 14 }}>Empieza a ahorrar en gasolina hoy</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          {[
            ...
            <div key={field.name}>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--color-muted)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{field.label}</label>
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type={showPass ? 'text' : 'password'} name="password" value={form.password} onChange={handleChange} placeholder="Mínimo 8 caracteres" style={{ ...inputStyle, paddingRight: 44 }}
                onFocus={e => e.target.style.borderColor = 'var(--color-accent)'}
                onBlur={e => e.target.style.borderColor = 'var(--color-border-strong)'} />
        ...
        <p style={{ textAlign: 'center', marginTop: 28, color: 'var(--color-muted)', fontSize: 14 }}>
          ¿Ya tienes cuenta?{' '}
          <Link to="/login" style={{ color: 'var(--color-accent)', textDecoration: 'none', fontWeight: 600 }}>Iniciar sesión</Link>
        </p>
      </div>
    </div>
  )
